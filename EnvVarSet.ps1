...
<# Get a list of the installed programs on the local machine using registry keys for generation. Once the list is generated, output the
   ...
   to the Oracle Java Platform, either JRE or JDK both 32-bit and 64-bit and store in variable. Note: There may be a way to simplify this subroutine.
#>
...
$jversion = $array | Where-Object { $_.DisplayName } | select ComputerName, DisplayName, DisplayVersion, Publisher |where {$_.DisplayName -like "Java*"} | ft -hidetableheaders
